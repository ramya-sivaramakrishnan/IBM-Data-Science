...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        title_txt='Total Success Launches By Site'
        fig = px.pie(filtered_df, values='class',names='Launch Site', title=title_txt)
    else:
        filtered_df=spacex_df[spacex_df['Launch Site']==entered_site]
        success=filtered_df['class'].sum()
        total=filtered_df['class'].count()
        failure=total-success
        values_computed=[success,failure]
        labels=['1','0']
        title_txt=(f'Total Success Launches For Site {entered_site}')
        fig = px.pie(filtered_df, values=values_computed,names=labels, title=title_txt)
    return fig
...
def get_scatter_plot(entered_site,payload_mass):
    select_min=payload_mass[0]
    select_max=payload_mass[1]
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)']>=select_min) & (spacex_df['Payload Mass (kg)']<=select_max)]
    if entered_site == 'ALL':
        title_txt='Correlation between Payload and Success for all Sites'
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',y='class', color='Booster Version Category',title=title_txt)
    else:
        filtered_df=spacex_df[spacex_df['Launch Site']==entered_site]
        title_txt=(f'Total Success Launches For Site {entered_site} for the payload range {payload_mass}')
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',y='class', color='Booster Version Category',title=title_txt)
    return fig
